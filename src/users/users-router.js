...
const encryptEmail = (email) => {
  ...
  const sliced = email.slice(1, indexOfAt - 1);
  const regx = new RegExp(sliced, 'gi');
  const hiddenEmail = email.replace(regx, '*'.repeat(sliced.length));
  return hiddenEmail;
};
...
usersRouter
  ...
  .post(bodyParser, (req, res, next) => {
    const {
      fname, lname, dob, email, password, marital_status, occupation, bio, gender,
    } = req.body;
    const requiredFields = {
      fname, lname, dob, email, password, marital_status, bio, gender,
    };
  ...
  .patch(requireAuth, bodyParser, (req, res, next) => {
    ...
    const {
      fname, lname, marital_status, occupation, bio, gender,
    } = req.body;
    const requiredFields = {
      fname, lname, marital_status, occupation, bio, gender,
    };
